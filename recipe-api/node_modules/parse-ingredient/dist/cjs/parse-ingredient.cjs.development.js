...
const numericRegexAnywhere = numeric_quantity.numericRegex.source.replace(/^\^/, "").replace(/\$$/, "");
/**
...
const parseIngredient = (ingredientText, options = defaultOptions) => {
	...
	return ingredientArray.map((line) => {
		...
		if (!isNaN((0, numeric_quantity.numericQuantity)(line[0])) || line[0] === "." && !isNaN((0, numeric_quantity.numericQuantity)(line.slice(0, 2)))) {
			let lenNum = 6;
			let nqResult = NaN;
			while (lenNum > 0 && isNaN(nqResult)) {
				nqResult = (0, numeric_quantity.numericQuantity)(line.substring(0, lenNum).trim());
				if (nqResult > -1) {
			...
			if (trailingQtyResult && opts.ignoreUOMs.includes((_trailingQtyResult$at = trailingQtyResult.at(-1)) !== null && _trailingQtyResult$at !== void 0 ? _trailingQtyResult$at : "")) oIng.description = line;
				...
				if (!firstQty) oIng.quantity = (0, numeric_quantity.numericQuantity)(secondQty);
				else {
					oIng.quantity = (0, numeric_quantity.numericQuantity)(firstQty);
					oIng.quantity2 = (0, numeric_quantity.numericQuantity)(secondQty);
				}
		...
		if (q2reMatch) {
			...
			const nqResultFirstChar = (0, numeric_quantity.numericQuantity)(oIng.description.substring(q2reMatchLen).trim()[0]);
			if (!isNaN(nqResultFirstChar)) {
				let lenNum = 7;
				let nqResult = NaN;
				while (--lenNum > 0 && isNaN(nqResult)) {
					nqResult = (0, numeric_quantity.numericQuantity)(oIng.description.substring(q2reMatchLen, lenNum));
					if (!isNaN(nqResult)) {
...
exports.trailingQuantityRegEx = trailingQuantityRegEx;
